...
app.post('/atualizar-aluno', (req, res) => {
    const { nome, sala, numero, materiaFav, sexo } = req.body
 
    const alunosIndex = alunos.findIndex(aluno => alunos.nome.toLowerCase() === nome.toLowerCase());
...
app.get('/excluir-alunos', (req, res) => {
...
app.get('/excluir-aluno-confirmado', (req, res) => {
    ...
    const alunosIndex = alunos.findIndex(alunos => aluno.nome.toLowerCase() === nome.toLowerCase());
...
app.get('/buscar-aluno/sala', (req, res) => {
    const salaDoAlunoBuscado = req.query.sala;
 
    if (!salaDoAlunoBuscado) {
        return res.send(`
            <h1>Por favor, insira uma sala de um aluno.</h1>
            <form action="/buscar-alunos" method="get">
                <label for="sala">sala do aluno:</label>
                <input type="number" name="sala"/>
                <button type="submit">Buscar aluno</button>
            </form>
        `);
    }
 
    function BuscarAlunoPorPreco(sala) {
        return alunos.find(aluno => parseFloat(alunos.sala) === parseFloat(sala));
    }
 
    const alunoEncontrado = BuscarAlunoPorSala(salaDoAlunoBuscado);
